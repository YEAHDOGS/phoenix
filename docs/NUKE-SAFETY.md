...
| 4 | **Boot-USB guard** | The booted USB is detected (mounted partitions + `/proc/cmdline` root device) and **refused structurally** — the script exits, it does not warn-and-continue. The disk inventory contract (`phoenix-disk-inventory/1`) also carries a per-disk `boot` flag — bash: parent of the kernel `root=`/`BOOT_IMAGE=` device (read-only `/proc/cmdline` inspection, `PHOENIX_MOCK_BOOT` override for tests); PowerShell: `Get-Disk` `IsBoot`/`IsSystem` — and both confirmation twins (`tools/lib/nuke-interlock.sh` `nuke_confirm_target`, `tools/Confirm-NukeTarget.ps1`) refuse a `boot`/`system` disk **before** the mounted-partition and TTY gates, so the booted disk can never be armed even by a script that only sources the gate library. |
