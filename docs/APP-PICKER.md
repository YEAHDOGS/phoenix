...
Installs user's chosen Windows apps automatically at first logon via Chocolatey.
Three moving parts:
...
34 packages are `defaultSelected: true` — the stuff user actually uses:
...
The build machine may be Linux (user's clean room is as likely to be a
