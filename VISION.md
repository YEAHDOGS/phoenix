...
- Windows ISO download with **SHA-512 verification** (extend
  `checksum/check.ps1`; user already verifies ISOs this way)
...
### Phase 5 - The GUI app
**Recommendation: Tauri v2 + Svelte 5 + Tailwind.** It's user's exact
...
user's `REDACTED/reviOS-offline` fork is the reference implementation
...
- The offline fork still trusts pre-staged binaries; the stager must
  ...
  user's existing practice) and re-verify from `manifest.json` on the
  target.
