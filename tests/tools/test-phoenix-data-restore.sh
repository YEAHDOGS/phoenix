...
mkdir -p "$SRC/Users/user/Documents" "$SRC/Users/user/Downloads" "$SRC/Users/user/Desktop"
echo "my manuscript" > "$SRC/Users/user/Documents/novel.txt"
echo "photo bytes"  > "$SRC/Users/user/Desktop/photo.png"
echo "game save"    > "$SRC/Users/user/Documents/save.dat"
printf 'MZfakeexe'  > "$SRC/Users/user/Downloads/setup.exe"
printf 'MZfakemsi'  > "$SRC/Users/user/Downloads/app.msi"
echo "legit script I wrote" > "$SRC/Users/user/Documents/notes.ps1"
echo "music" > "$SRC/Users/user/Documents/song.mp3"
...
"$BACKUP_SH" --source-dir "$SRC" --out "$OUT" --profiles user --operator testop \
...
echo "$RESTORED" | grep -qx 'Users/user/Documents/novel.txt' && pass "ref: novel.txt would be restored" || fail "ref: novel.txt"
echo "$RESTORED" | grep -qx 'Users/user/Desktop/photo.png'   && pass "ref: photo.png would be restored"  || fail "ref: photo.png"
echo "$RESTORED" | grep -qx 'Users/user/Documents/save.dat'  && pass "ref: save.dat would be restored"   || fail "ref: save.dat"
echo "$RESTORED" | grep -qx 'Users/user/Documents/song.mp3'  && pass "ref: song.mp3 would be restored"   || fail "ref: song.mp3"
echo "$REFUSED"  | grep -qx 'Users/user/Downloads/setup.exe' && pass "ref: setup.exe refused even though backup skipped it" || fail "ref: setup.exe"
echo "$REFUSED"  | grep -qx 'Users/user/Downloads/app.msi'   && pass "ref: app.msi refused"              || fail "ref: app.msi"
echo "$REFUSED"  | grep -qx 'Users/user/Documents/notes.ps1' && pass "ref: notes.ps1 refused (.ps1 is executable)" || fail "ref: notes.ps1"
...
cp -r "$OUT" "$T/tampered"
echo "evil bytes" >> "$T/tampered/Users/user/Documents/novel.txt"
python3 "$REFPY" "$RESTORE" "$T/tampered" > "$T/tamper.out"
grep -qx 'TAMPER Users/user/Documents/novel.txt' "$T/tamper.out" \
