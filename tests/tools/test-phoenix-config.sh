...
"$SH" --computer-name USER-PC --username user --password 's3cr"t\pass' \
...
assert d["machine"]["computerName"] == "USER-PC"
assert d["machine"]["timezone"] == "Central Standard Time"
assert d["credentials"]["username"] == "user"
...
[[ "$(jget "$OUT" computerName)" == "USER-PC" ]] && pass "menu jget: computerName" \
...
echo "== dry-run =="
DRYOUT="$("$SH" --computer-name USER-PC --username user --password 'hunter2-secret' \
