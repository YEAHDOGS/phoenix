...
- Windows ISO download with **SHA-512 verification** (extend
  `checksum/check.ps1`; the user already verifies ISOs this way)
...
### Phase 5 - The GUI app
**Recommendation: Tauri v2 + Svelte 5 + Tailwind.** It's the user's exact
...
the ReviOS-offline fork is the reference implementation
...
- The offline fork still trusts pre-staged binaries; the stager must
  ...
  the user's existing practice) and re-verify from `manifest.json` on the
  target.
